...
it.each([32, 48, 64, 88])(
  ...
  (size) => {
    ...
    const initial = screen.getByLabelText("learner's avatar");
...
it('centers a real image with cover mode and falls back safely on image failure', () => {
  ...
  const image = screen.getByLabelText("learner's avatar");
  expect(image).toHaveProp('resizeMode', 'cover');
  fireEvent(image, 'error');
  expect(screen.getByLabelText("learner's avatar")).toHaveTextContent('L');
...
it('renders the empty-username fallback without depending on a bundled image baseline', () => {
  ...
  expect(screen.getByLabelText("Langtify's avatar")).toHaveTextContent('L');
});

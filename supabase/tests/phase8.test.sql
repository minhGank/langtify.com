...
create temporary table pending_photo as
 select (public.reserve_submission((select id from public.daily_challenge_words where slot='target' and daily_challenge_id=(select daily_challenge_id from public.submissions where id=(select id from photos))))).id;
